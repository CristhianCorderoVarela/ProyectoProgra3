...
public class VistaSalonesController implements Initializable {
    ...
    private final Map<Long, StackPane> mapaMesasVista = new HashMap<>();
    ...
    private void cargarSalones() {
        try {
            ...
            System.out.println("RAW GET /salones/tipo/salon -> " + jsonResponse); // 👈 debug contrato
    ...
    private void cargarMesasSalon(Salon salon) {
        try {
            ...
            // Cargar imagen de mesa del salón (si viene como base64 en JSON, Gson mapea a byte[] sin adaptadores)
            ...
            System.out.println("RAW GET /salones/" + salon.getId() + "/mesas -> " + jsonResponse); // 👈 debug contrato
            Map<String, Object> response = RestClient.parseResponse(jsonResponse);
    ...
    private void mostrarMesasEnPanel(Salon salon) {
        ...
        if (salon.getMesas() == null || salon.getMesas().isEmpty()) {
            ...
            Label lblInstruccion = new Label(
                    I18n.isSpanish() ?
                            "Active el modo edición y haga clic derecho en el área vacía para agregar mesas" :
                            "Enable edit mode and right-click on empty area to add tables"
            );
            lblInstruccion.setStyle("-fx-text-fill: #666; -fx-font-size: 14px;");

            vboxMensaje.getChildren().addAll(lblSinMesas, lblInstruccion);
            vboxMensaje.setLayoutX(Math.max(20, panelMesas.getPrefWidth() / 2 - 200));
            vboxMensaje.setLayoutY(Math.max(20, panelMesas.getPrefHeight() / 2 - 40));
    ...
    private StackPane crearVistaMesa(Mesa mesa) {
        ...
        container.setLayoutX(safe(mesa.getPosicionX()));
        container.setLayoutY(safe(mesa.getPosicionY()));
        ...
        // Imagen base del salón (opcional)
    ...
    private void configurarMenuContextual(StackPane mesaPane, Mesa mesa) {
        ...
        mesaPane.setOnContextMenuRequested(e -> menu.show(mesaPane, e.getScreenX(), e.getScreenY()));
    }
        ...
        result.ifPresent(identificador -> {
            ...
            try {
                ...
                if (Boolean.TRUE.equals(response.get("success"))) {
                    ...
                    Mensaje.showError("Error", String.valueOf(response.get("message")));
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void actualizarUIModEdicion() {
        if (modoEdicion) {
            ...
            lblInfo.setText(I18n.isSpanish() ?
                    "Arrastre mesas para moverlas | Clic derecho: editar/eliminar | Clic derecho en área vacía: agregar mesa" :
                    "Drag tables to move | Right-click: edit/delete | Right-click on empty area: add table");

            // Clic derecho en el área vacía del panel para agregar mesas (robusto)
            panelMesas.setOnMouseClicked(e -> {
                if (e.getButton() == MouseButton.SECONDARY) {
                    if (!clickSobreMesa(e.getTarget())) {
    ...
    private boolean clickSobreMesa(Object target) {
        if (!(target instanceof Node)) return false;
        Node n = (Node) target;
        while (n != null) {
            if (n instanceof StackPane sp && sp.getUserData() instanceof Mesa) {
                return true;
            }
            n = n.getParent();
        }
    ...
    private void guardarDiseno() {
        if (salonActual == null || salonActual.getMesas() == null || salonActual.getMesas().isEmpty()) {
    ...
    private void guardarDisenoSilencioso() {
        if (salonActual == null || salonActual.getMesas() == null || salonActual.getMesas().isEmpty()) return;
